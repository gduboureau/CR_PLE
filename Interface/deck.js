...
class Deck {
  ...
  */
  constructor(deckstr = undefined) {
    ...
    if (deckstr !== undefined) {
      for (let i = 0; i < deckstr.length / 2; ++i) {
  ...
  */
  cards() {
    ...
    for (let i = 0; i < this._nbCard; ++i) {
      let c = Deck._int2Card[parseInt(this._deck[i], 16)]
      result.push([c.name, c.iconUrls.medium]);
    }
    return result;
  }
  /*
...
function nameToUrl(name) {
  if (_mapN2U === undefined) {
    ...
    for (let x in cr_cards) {
      let c = cr_cards[x];
      _mapN2U[c.name] = c.iconUrls.medium;
    }
  }
  return _mapN2U[name];
}

let _mapN2Id = undefined;
/*
